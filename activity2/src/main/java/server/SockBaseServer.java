...
class SockBaseServer {
    ...
    public static void main (String args[]) throws Exception {
        ...
        while (true) {
			final Socket cliente = serv.accept();
            pool.execute(() -> {
				try {
					SockBaseServer server = new SockBaseServer(cliente, game);
					server.start();
				} catch(Exception e) {
					e.printStackTrace();
					System.exit(2);
			...
			pool.execute(game.new Player(serv.accept(), 'X'));
            pool.execute(game.new Player(serv.accept(), 'O'));
        }
